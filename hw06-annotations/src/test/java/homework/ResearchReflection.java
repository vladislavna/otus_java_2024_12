...
import check.SimpleCustomerTest;
...
public class ResearchReflection {
    ...
    public static void main(String[] args) throws ClassNotFoundException {
        String nameOfTestClass = "check.SimpleCustomerTest";
    ...
    private static void runTests(String nameClass) throws ClassNotFoundException {
        ...
        if (!methodsTest.isEmpty()) {
            methodsTest.stream().forEach(method -> {
                Object inst;
                try {
    ...
    private static void runBeforeOrAfter(Object inst, List<Method> methodsAdditional) {
        if (!methodsAdditional.isEmpty()) {
            methodsAdditional.stream().forEach(method -> {
                try {
                    method.invoke(inst);
                } catch (Exception e) {
                    logger.atInfo()
